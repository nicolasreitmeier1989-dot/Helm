...
// first contact: "MISSION // ADVERSARIAL SIMULATION" → "PROJECT //
// COMPETITIVE ANALYSIS", "OPPONENT" → "COMPETITOR", "OWN" → "OUR COMPANY"
// in the headline chrome; deep technical labels inside the visualizations
// stay short and dense (still a strategist tool).
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    setSessionId(id);
    // Hydrate from the active project (set by landing/wizard). If none,
    // keep the seeded defaults so the dashboard is never empty.
    const active = getActiveProject();
    if (active) {
      setCompetitor(active.competitor);
      setOwn(active.own);
    ...
    <main className="min-h-screen bg-ink-0 text-ink-900">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[420px_1fr_420px] gap-px bg-ink-300/60">
        ...
        <aside className="bg-ink-0 p-4 space-y-4 max-h-[calc(100vh-12rem)] overflow-y-auto xl:sticky xl:top-12">
          ...
          {/* OUR COMPANY / COMPETITOR toggle for the topology editor */}
